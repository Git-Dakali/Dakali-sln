...
namespace DK.Validator.Sales
{
    public class SaleValidator
    {
        ...
        {
            await LogisticsProvider(sale, cancellationToken);
            ...
            foreach (var item in sale.SaleDetails)
                await _saleDetailValidator.Update(sale, item, cancellationToken);
        }

        public async Task Update(Sale sale, CancellationToken cancellationToken = default)
        {
            if (sale.State != SaleState.Creado && sale.State != SaleState.Confirmado && sale.State != SaleState.Preparado && sale.State != SaleState.PendienteDespachar)
                throw new Exception($"No se puede realizar cambios en estado {sale.State}.");
            ...
            if (sale.State != SaleState.Creado && sale.State != SaleState.Anulado)
            {
        ...
        public async Task LogisticsProvider(Sale sale, CancellationToken cancellationToken = default)
        {
            if (sale.LogisticsProvider is null)
                throw new Exception("Debe ingresar un Proveedor Logistico.");
        }
        ...
        public async Task SaleDetails(Sale sale, CancellationToken cancellationToken = default) 
        {
            ...
            if (sale.IsReverseLogistics && !sale.SaleDetails.Any(x => x.IsExchangeItem))
                throw new Exception("Debe ingresar un detalle para el cambio.");
            if (!sale.IsReverseLogistics && sale.SaleDetails.Any(x => x.IsExchangeItem))
                throw new Exception($"Posee un detalle para logistica inversa pero la venta {sale.Number} no esta marcado como una logistiva inversa.");
        }
    }
}
